class Virtualmachine < ActiveRecord::Base
  ...
  #Create VS
    ...
    :custom_recipe_variables => {
       ...
       ####### need to test the part of secondary network interface along the virtual machine creation ####
  ...
  def usageVMcall(idForCpu)
    ...
    vmusage_response = vmusage.cpu_usage(idForCpu,Time.now - 1.day,Time.now)
  ...
  def startVMcall(idForStart)
    ...
    vmstart_response = vmstart.startup(idForStart)
  ...
  def shutdownVMcall(idForShut)
    ...
    vmshut_response = vmshut.shutdown(idForShut)
  ...
  def rebootVMcall(idForBoot)
    ...
    vmboot_response = vmboot.reboot(idForBoot)
  ...
  def suspendVMcall(idForSuspend)
    ...
    vmsuspend_response = vmsuspend.suspend(idForSuspend)
  ...
  def rebuildVMcall(idForRebuild)
    ...
    vmrebuild_response = vmrebuild.rebuild(idForRebuild)
